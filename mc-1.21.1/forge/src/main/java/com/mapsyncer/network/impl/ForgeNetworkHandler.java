...
public class ForgeNetworkHandler implements NetworkHandler<ServerPlayer, Object> {
    ...
    public void init() {
        ...
        CHANNEL.messageBuilder(ForgeSyncAllowedMessage.class, 4, NetworkDirection.PLAY_TO_CLIENT)
            .encoder(ForgeSyncAllowedMessage::encode)
            .decoder(ForgeSyncAllowedMessage::decode)
            .consumerMainThread((msg, ctx) -> {
    ...
    public void sendToPlayer(ServerPlayer player, SyncAllowedPayload payload) {
        if (!confirmedPlayers.contains(player.getUUID())) return;
        CHANNEL.send(new ForgeSyncAllowedMessage(payload), PacketDistributor.PLAYER.with(player));
    }
