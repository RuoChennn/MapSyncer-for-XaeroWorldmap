...
/**
 * Fabric 1.20.1 专用服务端命令注册。
 *
 * 使用 {@code /mapsyncerserver} 前缀，避免与客户端 {@code /mapsyncer} 冲突。
 * 功能逻辑委托给 {@link CacheCommandHandler}。
 */
public class FabricServerCommand {

    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("mapsyncerserver")
                .requires(source -> source.hasPermission(4))
                .executes(FabricServerCommand::showHelp)
                .then(Commands.literal("help")
                        .executes(FabricServerCommand::showHelp))
                .then(Commands.literal("generate")
                        .executes(FabricServerCommand::generateAll)
                        .then(Commands.argument("dimension", DimensionArgument.dimension())
                                .executes(FabricServerCommand::generateDimension)
                                .then(Commands.literal("--force")
                                        .executes(FabricServerCommand::generateDimensionForce))
                                .then(Commands.argument("x", IntegerArgumentType.integer())
                                        .then(Commands.argument("z", IntegerArgumentType.integer())
                                                .executes(FabricServerCommand::generateSingleRegion)))))
                .then(Commands.literal("status")
                        .executes(FabricServerCommand::showStatus))
                .then(Commands.literal("incremental")
                        .then(Commands.literal("off")
                                .executes(FabricServerCommand::setIncrementalOff))
                        .then(Commands.literal("tick")
                                .executes(FabricServerCommand::setIncrementalTick)
                                .then(Commands.argument("interval", IntegerArgumentType.integer(20, 72000))
                                        .executes(FabricServerCommand::setIncrementalTickInterval)))
                        .then(Commands.literal("scheduled")
                                .executes(FabricServerCommand::setIncrementalScheduled)
                                .then(Commands.argument("hour", IntegerArgumentType.integer(0, 23))
                                        .executes(FabricServerCommand::setScheduledTimeDefaultMinute)
                                        .then(Commands.argument("minute", IntegerArgumentType.integer(0, 59))
                                                .executes(FabricServerCommand::setScheduledTime))))));
    }
