...
- ✅ **客户端时间戳缓存路径修复**: 解决单例缓存路径不更新的问题
  - 问题：ClientTimestampCache 单例使用错误路径初始化（`null/` 而非 `Multiplayer_localhost/`）
  - 修复：添加 `lastBaseDir` 路径追踪，当路径变化时重新初始化单例
  - 效果：立即重新同步现在能正确读取缓存，跳过相同区域

- ✅ **服务端旧版维度名兼容**: 兼容旧版本缓存中的 Minecraft 维度名
  - 问题：旧版本缓存使用 `overworld/` 目录，新版本使用 `null/` 目录
  - 修复：添加 `toXaeroDimension()` 转换，自动将旧格式转换为 Xaero 格式
  - 效果：服务端能正确匹配旧版本生成的缓存

- ✅ **服务端哈希回退计算**: 当缓存没有记录时从文件计算哈希
  - 问题：旧版本缓存没有 `generation_cache.properties`，导致服务端跳过所有区域
  - 修复：当 GenerationCache 没有记录时，从 ZIP 文件计算 CRC32 哈希并与客户端比较
  - 效果：即使没有生成缓存记录，也能正确判断是否需要同步

- ✅ **客户端目录逻辑简化**: 移除冗余的 `baseDir` 参数
  - 修复：新增 `getCurrentServerDirectory()` 方法直接返回 `Multiplayer_<serverIP>` 目录
  - 修复：`sendSyncRequest()` 不再需要传入 `baseDir`，内部自动获取
  - 效果：代码更清晰，目录获取逻辑统一

- ✅ **维度名显示改为原始 ID**: 移除友好名称显示
  - 修复：显示原始维度 ID（如 `minecraft:overworld`）而非中文名称（如 "主世界"）
  - 修复：删除未使用的 `getFriendlyName()` 方法
  - 效果：用户能明确知道正在同步的维度 ID
...
- ✅ **末地区块存在性检测修复**: 区分区块未生成与区块内虚空区域
  - 问题：之前末地空白区域填充 air/void 的处理范围过大，区块尚未生成的区域也被填充
  - 修复：添加 `chunkExists[32][32]` 数组追踪区块存在性
  - 修复：`processChunk` 方法标记已读取的区块
  - 修复：`serializeToXaeroFormat` 区分两种情况：
    - 区块不存在（尚未生成）→ 写入空 Tile（`tileMarker = -1`），客户端跳过渲染
    - 区块存在但像素为虚空 → 写入 `AIR + void`（渲染深紫色）
  - 坐标映射：一个 Tile（16x16 块）对应一个 Minecraft 区块
